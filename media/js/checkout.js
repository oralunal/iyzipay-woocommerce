...
const payment_methods = all_settings.paymentMethodData
  ? all_settings.paymentMethodData.iyzico_blocks_for_woocommerce_gateway
  : all_settings.iyzico_blocks_for_woocommerce_gateway || {}
...
Object.entries(payment_methods).forEach(([key, value]) => {
  ...
  const DescriptionComponent = () => {
    ...
    return window.wp.element.createElement(
      ...
      key === "iyzico" ? descriptionImage : null
    )
  }

  const Block_Gateway = {
    name: key,
    label: key === "iyzico" ? label : window.wp.element.createElement(LabelComponent, null),
